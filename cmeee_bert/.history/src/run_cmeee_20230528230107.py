...
from transformers import set_seed, BertTokenizer, Trainer, HfArgumentParser, TrainingArguments, BertLayer, BartTokenizer, BartModel, BertTokenizerFast
from transformers import RobertaTokenizer, RobertaModel, ErnieConfig, BertConfig, BartConfig
...
from model import *
from global_pointer_data_loader import *
...
def get_model_with_tokenizer(model_args):
    ...
    elif "ernie_linear" in model_args.head_type:
...
def main(_args: List[str] = None):
    ...
    parser = argparse.ArgumentParser(description='Some additional arguments for CMeEE')
    parser.add_argument('--layerwise', action='store_true', help='Whether to use layerwise learning rate')
    parser.add_argument('--lr_decay', type=float, default=0.9, help='Learning rate decay rate')
    parser.add_argument('--augment', action='store_true', help='Whether to use data augmentation')
    plus_args = parser.parse_args()
    train_args.lr_decay = plus_args.lr_decay
    ...
    # ===== Get datasets =====
    if not 'global' in model_args.head_type:
        if train_args.do_train:
            train_dataset = EEDataset(data_args.cblue_root, "train", data_args.max_length, tokenizer, for_nested_ner=for_nested_ner, augment=plus_args.augment)
            dev_dataset = EEDataset(data_args.cblue_root, "dev", data_args.max_length, tokenizer, for_nested_ner=for_nested_ner, augment=plus_args.augment)
        ...
        if train_args.do_train:
            data_root = data_root = data_args.cblue_root+"/CMeEE/"
            bert_model_path="../bert-base-chinese"
            tokenizer = BertTokenizerFast.from_pretrained(bert_model_path, do_lower_case=True)
            train_dataset = EntDataset(load_data(data_root+"CMeEE_train.json"), tokenizer=tokenizer)
            dev_dataset = EntDataset(load_data(data_root+"CMeEE_dev.json"), tokenizer=tokenizer)
            logger.info(f"Trainset: {len(train_dataset)} samples")
            logger.info(f"Devset: {len(dev_dataset)} samples")
        else:
            train_dataset = dev_dataset = None
    ...
    if "global" in model_args.head_type:
        compute_metrics = GlobalPointerComputeMetricsForNER()
    ...
    if "global" in model_args.head_type:
        if "ernie" not in model_args.head_type:
            optimizer = (set_optimizer(model, train_steps= (int(len(train_dataset) / train_args.per_device_train_batch_size) + 1) * train_args.num_train_epochs), None)
        else:
            optimizer = (None, None)
        trainer = Trainer(
            model=model,
    ...
    elif "bert" in model_args.head_type and plus_args.layerwise:
