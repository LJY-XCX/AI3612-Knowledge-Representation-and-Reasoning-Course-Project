...
from augmentation import get_synonyms
import random
...
class InputExample:
    def __init__(self, sentence_id: str, text: str, entities: List[dict] = None, augment=True):
        ...
        if self.entities is None:
            ...
            if self.augment:
                if random.random() < 0.15:
                    entity_idx = random.randint(0, len(self.entities) - 1)
                    start_idx = self.entities[entity_idx]["start_idx"]
                    end_idx = self.entities[entity_idx]["end_idx"]
                    synonym = get_synonyms(self.text[start_idx: end_idx + 1])
                    self.text = self.text.replace(self.text[start_idx: end_idx + 1], synonym)

                if random.random() < 0.1:
                    entity_idx = random.randint(0, len(self.entities) - 1)
                    start_idx = self.entities[entity_idx]["start_idx"]
                    end_idx = self.entities[entity_idx]["end_idx"]
                    entity = self.text[start_idx: end_idx + 1]

                    seg_list = jieba.cut(self.text, cut_all=False)
                    seg_list = list(seg_list)
                    try:
                        seg_index = seg_list.index(entity)
                        if seg_index != 0:
                            seg_list[seg_index - 1] = get_synonyms(seg_list[seg_index - 1])
                        if seg_index != len(seg_list) - 1:
                            seg_list[seg_index + 1] = get_synonyms(seg_list[seg_index + 1])
                        self.text = ''.join(seg_list)
...
class EEDataloader:
    def __init__(self, cblue_root: str, augment=True):
    ...
    @staticmethod
    def _parse(cmeee_data: List[dict], augment) -> List[InputExample]:
        if augment:
            augmented_data =  [InputExample(sentence_id=str(i), **data, augment=False) for i, data in enumerate(cmeee_data)] + [
                InputExample(sentence_id=str(i), **data, augment=True) for i, data in enumerate(cmeee_data)]
            return list(set(augmented_data))
        else:
            return [InputExample(sentence_id=str(i), **data, augment=False) for i, data in enumerate(cmeee_data)]

    def get_data(self, mode: str):
        if mode == "select_dev":
            return self._parse(self._load_json(join(self.data_root, f"select_dev.json")), self.augment)
        if mode not in ("train", "dev", "test"):
            raise ValueError(f"Unrecognized mode: {mode}")
        return self._parse(self._load_json(join(self.data_root, f"CMeEE_{mode}.json")), self.augment)


class EEDataset(Dataset):
    def __init__(self, cblue_root: str, mode: str, max_length: int, tokenizer, for_nested_ner: bool, augment=True):
        self.cblue_root = cblue_root
        self.data_root = join(cblue_root, "CMeEE")
        ...
        if False and exists(cache_file):
            ...
            if augment:
                self.examples = EEDataloader(cblue_root, augment=True).get_data(mode)  # get original data
            else:
                self.examples = EEDataloader(cblue_root, augment=False).get_data(mode)  # get original data
    ...
    def _preprocess(self, examples: List[InputExample], tokenizer) -> list:
        ...
        for example in examples:
            if is_test:
                ...
                if self.for_nested_ner:
                    label = [(l1, l2) for l1, l2 in zip(label[0], label[1])]
            ...
            for word, L in zip(text, label):
                ...
                if not is_test:
                    if self.for_nested_ner:
                        label_ids[0].extend([label2id[0][L[0]]] + [tokenizer.pad_token_id] * (len(token) - 1))
                        label_ids[1].extend([label2id[1][L[1]]] + [tokenizer.pad_token_id] * (len(token) - 1))
                    else:
...
if __name__ == '__main__':
    ...
    MODEL_NAME = "/home/jiyu/ClearGrasp/pytorch_networks/cmeee/bert-base-chinese"
    CBLUE_ROOT = "/home/jiyu/ClearGrasp/pytorch_networks/cmeee/data/CBLUEDatasets"

    tokenizer = BertTokenizer.from_pretrained(MODEL_NAME)
    dataset = EEDataset(CBLUE_ROOT, mode="dev", max_length=10, tokenizer=tokenizer, for_nested_ner=True, augment=True)
